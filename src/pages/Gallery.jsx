import React from 'react'
import useFancybox from "../hooks/useFancybox.js";

function Gallery() {
  const [fancyboxRef] = useFancybox({

  });
  return (
    <>
      <section className="inner-page-header position-relative overflow-hidden">
        <img data-aos-duration="1000" data-aos="zoom-out-up" className="absolute-bg object-fit-cover" src="/public/assets/images/card-1.png" />
        <div className="bg-overly w-100 h-100 position-absolute top-0 start-0 pe-none" />
        <div className="container">
          <div className="row">
            <div className="col-12">
              <h1 data-aos-duration="1000" data-aos="fade-up" className="fs-70 text-center text-white">Gallery</h1>
            </div>
          </div>
        </div>
      </section>

      <section>
        <div className="container pt-100 pb-5">
          <div className="row">
            <div className="col-12">
              <div ref={fancyboxRef} className='fancybox-gallery d-grid grid-cols-3 gap-2'>
                <a data-fancybox="gallery" href="/assets/images/card-1.png">
                  <img src="/assets/images/card-1.png" alt="" />
                </a>
               <a data-fancybox="gallery" href="/assets/images/card-2.png">
                  <img src="/assets/images/card-2.png" alt="" />
                </a>
                <a data-fancybox="gallery" href="/assets/images/card-3.png">
                  <img src="/assets/images/card-3.png" alt="" />
                </a>
                <a data-fancybox="gallery" href="/assets/images/card-4.png">
                  <img src="/assets/images/card-4.png" alt="" />
                </a>
                <a data-fancybox="gallery" href="/assets/images/hero-2.png">
                  <img src="/assets/images/hero-2.png" alt="" />
                </a>
               <a data-fancybox="gallery" href="/assets/images/hero-3.png">
                  <img src="/assets/images/hero-3.png" alt="" />
                </a>
                <a data-fancybox="gallery" href="/assets/images/hero-4.png">
                  <img src="/assets/images/hero-4.png" alt="" />
                </a>
                <a data-fancybox="gallery" href="/assets/images/hero-section-image.png">
                  <img src="/assets/images/hero-section-image.png" alt="" />
                </a>
                 <a data-fancybox="gallery" href="/assets/images/card-2.png">
                  <img src="/assets/images/card-2.png" alt="" />
                </a>
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  )
}

export default Gallery